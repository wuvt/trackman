...
def get_groups(user_info):
    ...
    dirapi = googleapiclient.discovery.build(
...
def handle_authorize(remote, token, user_info):
    ...
    if (
        len(current_app.config["GOOGLE_ALLOWED_DOMAINS"]) > 0
        and not user_info["hd"] in current_app.config["GOOGLE_ALLOWED_DOMAINS"]
    ):
...
def handle_security_event(token):
    ...
    for event in claims["events"][risc_prefix + "/sessions-revoked"]:
        current_app.logger.info(
            "RISC: Sessions revoked for user {0}".format(event["subject"]["sub"])
        )
        current_app.auth_manager.end_all_sessions_for_user(event["subject"]["sub"])

    for event in claims["events"][risc_prefix + "/tokens-revoked"]:
        current_app.logger.info(
            "RISC: Tokens revoked for user {0}".format(event["subject"]["sub"])
        )
        current_app.auth_manager.end_all_sessions_for_user(event["subject"]["sub"])
